...
import { fetchPricing, meterGrokHome, meterRollouts, priceProviderAttempts, USAGE_KEYS, type MeteredRollouts, type PricingSnapshot } from "./usage";
...
export async function buildReportUnlocked(runDirectory: string, options: ReportOptions = {}): Promise<{ jsonPath: string; markdownPath: string }> {
  ...
  if (state.mentor) for (const arm of ARMS) {
    const metered = usage[arm];
    if (!metered) continue;
    const diagnostic = diagnosticsByArm?.[arm];
    const hasCacheWrites = metered.totals.cache_write_input_tokens > 0;
    const priced = diagnostic?.status === "valid" && !hasCacheWrites
      ? priceProviderAttempts(diagnostic.metrics, pricing) : null;
    ...
    metered.totals.estimated_api_usd = priced;
    metered.complete = metered.complete && priced !== null;
    metered.warnings.push(priced === null
      ? "mentor provider-attempt cost is unknown: validated usage, price, or cache-write accounting is unavailable"
      : "mentor cost uses validated provider attempts and actual models; rollout per-agent prices are withheld");
  }
