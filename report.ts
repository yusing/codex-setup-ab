...
export async function buildReportUnlocked(runDirectory: string, options: ReportOptions = {}): Promise<{ jsonPath: string; markdownPath: string }> {
  ...
  const dualDiagnosticSummary = state.mentor ? `This is the ${state.mentor.setup} setup's Mekugi-only mentor comparison: A / stock means mentor **off**; B / current means mentor **on**. Both run the same parent and child role configuration. Provider models below come from validated per-arm capture; a missing or inconsistent lineage suppresses a complete measurement. Capture files remain executor-writable, so this is consistency evidence, not tamper-proof provenance.\n\n| Arm | capture | root provider models | child provider models | logical requests | provider attempts |\n| --- | --- | --- | --- | ---: | ---: |\n${ARMS.map(arm => {
    ...
    return `| ${arm} | ${item?.status ?? "unavailable"} | ${item?.model_schedule?.root.join(", ") ?? "unknown"} | ${item?.model_schedule?.child.join(", ") ?? "unknown"} | ${requests.logical ?? "unknown"} | ${requests.provider_attempts ?? "unknown"} |`;
  }).join("\n")}\n\n` : "";
  const judgeSummary = state.judge
    ? `${valid
      ...
      ? `Two independent reversed-order passes ${state.judge.agreement ? "agreed" : "disagreed"}. Judge winner: **${state.judge.winner}**.${state.judge.disagreement ? ` ${state.judge.disagreement}` : ""}`
