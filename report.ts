...
function resultHasAllChecks(result: ArmResult | undefined, criteria?: RunState["criteria"]): boolean {
  ...
  if (criteria && ![1, 2].every(pass => {
    const semantic = result.grade!.semantic;
    const assessed = semantic?.[`pass-${pass}`];
    const fixed = semantic?.[`pass-${pass}-fixed`];
    const existing = semantic?.[`pass-${pass}-existing`];
    return assessed !== undefined && fixed !== undefined && existing !== undefined &&
      assessed.length === criteria.contract.criteria.length && fixed.length === (criteria.contract.black_box?.length ?? 0) &&
      existing.length === 1 && assessed.every(item => item.status !== "unassessed" && (item.basis === "executed" || item.status === "fail")) &&
      [...fixed, ...existing].every(item => item.status !== "unassessed" && item.basis === "executed");
  })) return false;
...
export async function buildReportUnlocked(runDirectory: string, options: ReportOptions = {}): Promise<{ jsonPath: string; markdownPath: string }> {
  ...
  const report = {
    ...
    source: state.source,
    setup: { ...state.execution, comparison: state.comparison ?? "stock-current", mekugi_flags: state.mekugi_flags ?? [], resource_limits: state.resource_limits, snapshot_manifest: state.snapshot_manifest },
