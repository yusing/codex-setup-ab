...
Use `scripts/run.sh --preset stock-mekugi --model gpt-6-sol --reasoning-effort high` to run a Sol/high pair without mentor handoff, or pass the same options directly to `prepare`. The default remains Astra/medium for the NVM task. The selected image must contain the matching host Codex and code-mode-host binaries; the runner checks their versions and hashes before inference. Build the selected Mekugi executable before rerunning.
